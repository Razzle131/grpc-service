...
// getHostnameCmd represents the getHostname command
var getHostnameCmd = &cobra.Command{
	Use:   "getHostname",
	Short: "gets hostname of the server",
...
func init() {
	rootCmd.AddCommand(getHostnameCmd)

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	// getHostnameCmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// getHostnameCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
